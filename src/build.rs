...
fn main() {
    ...
    // Write the bin entry shim to gen/main_entry.rs.
    // Cargo.toml's [[bin]] points there — it doesn't accept env vars in `path`,
    // so a stable project-relative location is needed. The directory is gitignored.
    generate_bin_entry();
...
/// Write `gen/main_entry.rs` — a tiny shim delegating to `homunc::main_hom::main()`
/// (compiled from `src/main.hom`). Replaces the hand-maintained `src/main.rs`.
fn generate_bin_entry() {
    // Multi-line body matches rustfmt's canonical form so `cargo fmt` is a no-op.
    let shim = "// Auto-generated by src/build.rs — do not edit.\n\
                // Bin entry: delegates to main_hom::main(), compiled from src/main.hom.\n\
                fn main() {\n    homunc::main_hom::main();\n}\n";
    let dir = PathBuf::from("gen");
    std::fs::create_dir_all(&dir).expect("failed to create gen/");
    let path = dir.join("main_entry.rs");
    let needs_write = std::fs::read_to_string(&path)
        .map(|existing| existing != shim)
        .unwrap_or(true);
    if needs_write {
        std::fs::write(&path, shim).expect("failed to write gen/main_entry.rs");
    }
    // Rerun build.rs if the shim is deleted or hand-edited so it gets recreated.
